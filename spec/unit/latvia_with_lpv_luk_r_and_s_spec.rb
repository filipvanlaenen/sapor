...
describe Sapor::LatviaWithLpvLukRAndS, '#area_code' do
  it 'returns LV∪{LPV,LuK,R,S} as the area code' do
    expect(Sapor::LatviaWithLpvLukRAndS.instance.area_code).to eq('LV∪{LPV,LuK,R,S}')
  end
end

describe Sapor::LatviaWithLpvLukRAndS, '#no_of_seats' do
  it 'returns 100 as the number of seats' do
    expect(Sapor::LatviaWithLpvLukRAndS.instance.no_of_seats).to eq(100)
  end
end

describe Sapor::LatviaWithLpvLukRAndS, '#population_size' do
  it 'returns a population size of 839,000' do
    expect(Sapor::LatviaWithLpvLukRAndS.instance.population_size).to eq(839_000)
  end
end

describe Sapor::LatviaWithLpvLukRAndS, '#seats' do
  it 'calculates the number of seats for the election of 2018 correctly' do
    LatviaWithLpvLukRAndS = Sapor::LatviaWithLpvLukRAndS.instance
    results = LatviaWithLpvLukRAndS.overall_election_results_of_2018
    seats = LatviaWithLpvLukRAndS.seats(results)
    expect(seats['Sociāldemokrātiskā partija “Saskaņa”']).to eq(23 - 8)
    expect(seats['Stabilitātei!']).to eq(23 - 8)
    ...
    expect(seats['Republika']).to eq(16 - 6)
    ...
    expect(seats['Attīstībai/Par!']).to eq(13 - 6)
