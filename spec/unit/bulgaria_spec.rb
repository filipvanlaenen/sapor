...
describe Sapor::Bulgaria, '#seats' do
  it 'calculates the number of seats for the election of 2017' do
    ...
    expect(seats['ПП ГЕРБ']).to eq(88 + 1) # Data mismatch in Сливен
    expect(seats['БСП за БЪЛГАРИЯ']).to eq(73 + 1 + 1 + 1) # Data mismatch in Враца, Плевен and Пловдив Град
    expect(seats['ОБЕДИНЕНИ ПАТРИОТИ – НФСБ, АТАКА и ВМРО']).to eq(24 - 1) # Data mismatch in Пловдив Град
    expect(seats['Движение за права и свободи – ДПС']).to eq(20 + 1 + 1) # Data mismatch in Сливен and София Област
