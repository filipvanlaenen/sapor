...
describe Sapor::EuropeanUnion720France, '#area_code' do
  it 'returns EU720[FR] as the area code' do
    expect(Sapor::EuropeanUnion720France.instance.area_code).to eq('EU720[FR]')
  end
end

describe Sapor::EuropeanUnion720France, '#no_of_seats' do
  it 'returns 81 as the number of seats' do
    expect(Sapor::EuropeanUnion720France.instance.no_of_seats).to eq(81)
  end
end

describe Sapor::EuropeanUnion720France, '#seats' do
  it 'calculates the number of seats using the votes of the 2014 election' do
    results = { 'Front national–Rassemblement bleu Marine  (ENF)' => 4_712_461,
                'Union pour un mouvement populaire (EPP)' => 3_943_819,
                'Parti socialiste–Parti radical de gauche (S&D)' => 2_650_357,
                ...
                'Force vie' => 138_708, 'Europe citoyenne (—)' => 127_849,
                'Parti du vote blanc (—)' => 110_090 }
    seats = Sapor::EuropeanUnion720France.instance.seats(results)
    expect(seats['Front national–Rassemblement bleu Marine  (ENF)']).to eq(22 + 1 + 1)
    expect(seats['Union pour un mouvement populaire (EPP)']).to eq(18 + 2)
    expect(seats['Parti socialiste–Parti radical de gauche (S&D)']).to eq(12 + 1 + 1)
    expect(seats['L’Alternative (ALDE)']).to eq(9)
    expect(seats['Europe Écologie Les Verts (Greens/EFA)']).to eq(8)
    expect(seats['Front de gauche–L’Union pour les Outremer (GUE/NGL)']).to \
      eq(5 + 1)
