...
describe Sapor::LatviaWithLpvLukNstRSAndSv, '#area_code' do
  it 'returns LV∪{LPV,LuK,NST,R,S,SV} as the area code' do
    expect(Sapor::LatviaWithLpvLukNstRSAndSv.instance.area_code).to eq('LV∪{LPV,LuK,NST,R,S,SV}')
  end
end

describe Sapor::LatviaWithLpvLukNstRSAndSv, '#no_of_seats' do
  it 'returns 100 as the number of seats' do
    expect(Sapor::LatviaWithLpvLukNstRSAndSv.instance.no_of_seats).to eq(100)
  end
end

describe Sapor::LatviaWithLpvLukNstRSAndSv, '#population_size' do
  it 'returns a population size of 839,000' do
    expect(Sapor::LatviaWithLpvLukNstRSAndSv.instance.population_size).to eq(839_000)
  end
end

describe Sapor::LatviaWithLpvLukNstRSAndSv, '#seats' do
  it 'calculates the number of seats for the election of 2018 adjusted to 2022 seats correctly' do
    LatviaWithLpvLukNstRSAndSv = Sapor::LatviaWithLpvLukNstRSAndSv.instance
    results = LatviaWithLpvLukNstRSAndSv.overall_election_results_of_2018
    seats = LatviaWithLpvLukNstRSAndSv.seats(results)
