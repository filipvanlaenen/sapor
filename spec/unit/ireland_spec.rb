...
describe Sapor::Ireland, '#no_of_seats' do
  it 'returns 158 as the number of seats' do
    expect(Sapor::Ireland.instance.no_of_seats).to eq(158)
  end
end
...
describe Sapor::Ireland, '#seats' do
  it 'calculates the number of seats for the election of 2016 correctly' do
    ...
    expect(seats['Fine Gael']).to eq(49 + 4)
    expect(seats['Fianna Fáil']).to eq(44 + 8)
    expect(seats['Sinn Féin']).to eq(23 - 7)
    ...
    expect(seats['Social Democrats']).to eq(3 + 1)
    ...
    expect(seats['Non-Party']).to eq(20 + 3)
  end
end
