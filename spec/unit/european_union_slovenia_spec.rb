...
describe Sapor::EuropeanUnion720Slovenia, '#area_code' do
  it 'returns EU720[SI] as the area code' do
    expect(Sapor::EuropeanUnion720Slovenia.instance.area_code).to eq('EU720[SI]')
  end
end

describe Sapor::EuropeanUnion720Slovenia, '#no_of_seats' do
  it 'returns 9 as the number of seats' do
    expect(Sapor::EuropeanUnion720Slovenia.instance.no_of_seats).to eq(9)
  end
end

describe Sapor::EuropeanUnion720Slovenia, '#seats' do
  it 'calculates the number of seats for the election of 2014' do
    results = { 'Kacin Konkretno (*)' => 19_659,
                'Državljanska lista (ALDE)' => 4_570,
                'Združena levica (GUE/NGL)' => 21_898,
                ...
                'Pozitivna Slovenija (*)' => 26_510,
                'Zares (*)' => 3_794 }
    seats = Sapor::EuropeanUnion720Slovenia.instance.seats(results)
    expect(seats['Kacin Konkretno (*)']).to eq(0)
    expect(seats['Državljanska lista (ALDE)']).to eq(0)
    expect(seats['Združena levica (GUE/NGL)']).to eq(0)
    expect(seats['Slovenska nacionalna stranka (*)']).to eq(0)
