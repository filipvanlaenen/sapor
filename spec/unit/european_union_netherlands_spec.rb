...
describe Sapor::EuropeanUnion720Netherlands, '#area_code' do
  it 'returns EU720[NL] as the area code' do
    expect(Sapor::EuropeanUnion720Netherlands.instance.area_code).to eq('EU720[NL]')
  end
end

describe Sapor::EuropeanUnion720Netherlands, '#no_of_seats' do
  it 'returns 31 as the number of seats' do
    expect(Sapor::EuropeanUnion720Netherlands.instance.no_of_seats).to eq(31)
  end
end

describe Sapor::EuropeanUnion720Netherlands, '#seats' do
  it 'calculates the number of seats for the election of 2014' do
    results = { 'Democraten 66 (ALDE)' => 735_825,
                'Christen-Democratisch Appèl–ChristenUnie–Staatkundig' \
                  ' Gereformeerde Partij (EPP)' => 721_766 + 364_843,
                ...
                'Aandacht en Eenvoud (—)' => 3_174,
                'IQ, de Rechten-Plichten-Partij (—)' => 1_705 }
    seats = Sapor::EuropeanUnion720Netherlands.instance.seats(results)
    expect(seats['Democraten 66 (ALDE)']).to eq(4 + 1)
    expect(seats['Christen-Democratisch Appèl–ChristenUnie–Staatkundig' \
                   ' Gereformeerde Partij (EPP)']).to eq(5 + 2 + 1)
    expect(seats['Partij voor de Vrijheid (ENF)']).to eq(4)
    expect(seats['Volkspartij voor Vrijheid en Democratie (ALDE)']).to eq(3 + 1)
    expect(seats['Socialistische Partij (GUE/NGL)']).to eq(2 + 1)
    expect(seats['Partij van de Arbeid–GroenLinks (S&D)']).to eq(3 + 2)
    expect(seats['Partij voor de Dieren (GUE/NGL)']).to eq(1)
    expect(seats['50Plus (ALDE)']).to eq(0 + 1)
