...
TARGET = '../../lib/sapor/regional_data/united_kingdom-20170608-tig.psv'.freeze
...
File.open(TARGET, 'w') do |output|
  ...
  output.puts '#'
  output.puts '# Changes:'
  output.puts '# Added The Independent Group as a combination of Conservative Party (3/318) and Labour Party (8/262)'
  output.puts '#'
  File.open(SOURCE).each do |line|
    ...
    if region == 1
      ...
      unless tig == 0
        output.puts "#{name} | The Independent Group | #{tig}"
      end      
    end
